...
#         'gaepytz', os.path.join(os.path.dirname(__file__), 'locales'),
#         fallback=True
#         )
# def _(timezone_name):
...
class _FixedOffset(datetime.tzinfo):
    ...
    def __repr__(self):
        return 'gaepytz.FixedOffset(%d)' % self._minutes
...
def FixedOffset(offset, _tzinfos = {}):
    """return a fixed-offset timezone based off a number of minutes.
        ...
        >>> one = FixedOffset(-330)
        >>> one
        gaepytz.FixedOffset(-330)
        >>> one.utcoffset(datetime.datetime.now())
        datetime.timedelta(-1, 66600)

        >>> two = FixedOffset(1380)
        >>> two
        gaepytz.FixedOffset(1380)
...
def _test():
    ...
    import gaepytz
    return doctest.testmod(gaepytz)
